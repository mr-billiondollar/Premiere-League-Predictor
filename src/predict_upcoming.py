...
def load_xgb_model(name: str) -> XGBClassifier:
    """
    Load an XGBoost model saved with model.save_model() (native JSON/UBJ
    format), NOT joblib/pickle. Native format is what train_model.py
    actually writes, and it's immune to the "input stream corrupted" /
    version-mismatch errors that plain pickle can throw when the xgboost
    version that saved the file differs from the one loading it.
    """
    path = MODELS_DIR / f"{name}.json"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing {path}. Run `python src/train_model.py` first to "
            f"generate the trained models."
        )
    model = XGBClassifier()
...
def main(days_ahead: int = 10):
    ...
    xgb_unweighted = load_xgb_model("xgboost_unweighted")
    xgb_balanced = load_xgb_model("xgboost_balanced")
